...
void overlaying_image(SDL_Surface *image, SDL_Surface *green_nb, int start,
                      int end) {
  ...
  SDL_Surface *resize_nbG =
      SDL_CreateRGBSurfaceWithFormat(0, Wg, Hg, 32, format->format);
  ...
  for (int x = start_x; x < end % width; x++) {
    for (int y = start_y; y < end / width; y++) {
      SDL_GetRGBA(pixtab2[(x - start_x) + (y - start_y) * Wg], format, &r1, &g1,
                  &b1, &a1);
      if (g1 != 0) { // verify if the green_nb is green if not it is black
