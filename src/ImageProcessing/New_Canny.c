        ...
        , int* kernel, int kernelWidth)
{
    ...
    for (int j = y-(kernelWidth/2); j <= y+(kernelWidth/2); j++)
    {
        ...
        {
            sum_coef += kernel[(j-(y-(kernelWidth/2)))*kernelWidth+(i-
                    (x-(kernelWidth/2)))];
            int coef_act = kernel[(j-(y-(kernelWidth/2)))*kernelWidth+(i-
                    (x-(kernelWidth/2)))];
...
// possible ajustement des bornes pour plus ou moins de precision sur les bords de l'image
    ...
    for (int y = 2; y < height-2; y++) 
    {
        for (int x = 2; x < width-2; x++) 
        {
            ...
            new_pixtab[y * width + x] = SDL_MapRGBA(format, (Uint8)grad
...
static Uint32 Sup_max_calculus(Uint8 Intensity_Gradian
        ,Uint8 Orientation_Gradian, SDL_Surface* surface, int i, int j)
{
    ...
    case 135:
        if (j+1<height && i+1<height)
            SDL_GetRGBA(pixtab[(j+1) * width + i+1],format,&intensity1,&bin
                    ,&bin,&bin);
        if (j-1>=0 && i-1>=0)
            SDL_GetRGBA(pixtab[(j-1) * width + i-1],format,&intensity2,&bin
                    ,&bin,&bin);
        break;
    ...
    return SDL_MapRGBA(format, (Uint8)Intensity_Gradian, 
            (Uint8)Intensity_Gradian, (Uint8)Intensity_Gradian, (Uint8)255);
}
        ...
        SDL_Surface* Orientation_Gradian_Image)
{
    ...
    for (int y = 0; y < height; y++) 
    {
        for (int x = 0; x < width; x++) 
        {
            ...
            SDL_GetRGBA(pixtab_orientation[y * width + x], format, 
                    &orientation, &trash1, &trash2, &trash3);

            new_pixtab[y * width + x] = Sup_max_calculus(intensity, orientation
...
void Thresholdhysteresis(SDL_Surface* image)
{
    ...
    for (int y = 0; y < height; y++) 
    {
        for (int x = 0; x < width; x++) 
        {
            ...
            pixtab[y * width + x] = SDL_MapRGBA(format, (Uint8)col, (Uint8)col
                    , (Uint8)col, 255);
        }
    }
        ...
        for (int x = 0; x < width; x++) 
        {
            ...
            if (col == 127)
            {
                ...
                if (max == 255)
                    ...
                    col =0;
                pixtab[y * width + x] = SDL_MapRGBA(format, (Uint8)col, 
                        (Uint8)col, (Uint8)col, 255);
            }
            else
                continue;
        }
...
Uint8 ComputeSeuil(SDL_Surface* image)
{
    ...
    for (int i=0; i<256; ++i) 
    {
        ...
        double var = mean_diff * mean_diff *(double)weight_back 
            * (double)weight_fore;
        if (var>max) 
        {
            max = var;
